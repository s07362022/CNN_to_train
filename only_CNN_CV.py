...
IMAGE_SIZE =32
train_list = []
train_ary= np.zeros(shape=(100,32,32,3))

#載入模型
#model = Sequential()
model = keras.models.load_model('D:\\Python\\project1\\CNN4.h5') 
...
def CatchUsbVideo(window_name, camera_idx):
    ...
    while cap.isOpened():
        ...
        if face[a] == 1:                                                        
            #cv2.rectangle(frame)
            #文字提示是誰
            cv2.putText(frame,'linientsu' ,(10, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 1, cv2.LINE_AA)                                     
        else:
            pass
