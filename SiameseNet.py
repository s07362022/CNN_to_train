...
def predict(model_path, image_path1, image_path2, target_size,i):
    ...
    pred = y_pred.ravel() < 0.5
    print("第 %d 個 pred: " % str(i) ,pred)  # 相似程度
...
#plt.show()
#####
def plot_image(image,labels,prediction,idx,num=25):  
    fig = plt.gcf() 
    ...
    for i in range(0, num): 
        ...
        idx+=1 
    plt.show() 
plot_image(te_pairs[:25,1],te_y[:25],pre[:10],idx=0)

plot_image(te_pairs[:25,0],te_y[:25],pre[:10],idx=0)
